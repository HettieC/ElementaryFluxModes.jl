...
efm_1 = Dict(x => y for (x,y) in zip(reactions(model.inner)[row_order],E[:,1]))
efm_2 = Dict(x => y for (x,y) in zip(reactions(model.inner)[row_order],E[:,2]))


for (i,e) in enumerate([efm_1, efm_2])#, efm_3, efm_4])
    open("data/emf_N1w$(i).json","w") do io 
        JSON.print(io,e)
    end
end
